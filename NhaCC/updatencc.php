...
<body>
    <?php
        ...
        $sql = "select *from nhacungcap where MaNCC = $khoa";
        ...
        if(isset($_POST["btnUpdate"])){
            ...
            $sdt = $_POST["txtSDT"];
            $sql = "UPDATE `nhacungcap` SET `TenNCC` = '$tenncc', `DiaChi` = '$diachi', `Email` = '$email', `SDT` = '$sdt'  WHERE `nhacungcap`.`MaNCC` = '$khoa'";
            $result = mysqli_query($conn, $sql);
            if($result){
                mysqli_close($conn);
    ...
    <div class="container col-md-5" style="font-family: sans-serif; font-weight: bold;">
		<div class="card">
			<div class="card-body">
                <h3 style="text-align: center; color: blue;">Update nhà cung cấp</h3>
                <hr size="3px" color="red">
				<fieldset class="form-group">
					<label>Mã nhà cung cấp: </label> 
                    <input type="text" name="txtMancc" value = "<?php echo $row['MaNCC']?>" class="form-control" required="required">
				</fieldset>

				<fieldset class="form-group">
					<label>Tên nhà cung cấp: </label> 
                    <input type="text" name="txtTenNCC" value = "<?php echo $row['TenNCC']?>" class="form-control" required="required">
				</fieldset>

                <fieldset class="form-group">
					<label>Địa chỉ: </label> 
                    <input type="text" name="txtDiaChi" value = "<?php echo $row['DiaChi']?>" class="form-control" required="required">
				</fieldset>

                <fieldset class="form-group">
					<label>Số điện thoại: </label> 
                    <input type="text" name="txtSDT" value = "<?php echo $row['SDT']?>" class="form-control" required="required">
				</fieldset>

                <fieldset class="form-group">
					<label>Email: </label> 
                    <input type="text" name="txtEmail" value = "<?php echo $row['Email']?>" class="form-control" required="required">
				</fieldset>
